...
@Component({
  ...
  template: `
    <div class="page">
      <header class="intro">
        <h1>LinkedIn</h1>
        <p>
          A section-by-section review of your profile, and a check that it agrees with
          your <a routerLink="/profile">career profile</a>. Runs without a language
          model, like the CV analysis.
        </p>
      </header>
      ...
      <section class="card">
        <div class="card-header">
          ...
          <span class="card-note">
            CV Pal never opens linkedin.com — you bring the copy.
          </span>
        </div>

        <p class="body muted">
          <strong>Print your profile to PDF</strong> (open your profile, expand every
          "see more", then Ctrl+P → Save as PDF). That works on every account and takes
          about a minute. LinkedIn's own <em>Save to PDF</em> works too where it is
          offered, and so does the ZIP from a full data export.
        </p>
        ...
        @if (pasting()) {
          <div class="paste">
            ...
            <span class="muted small">
              Pasting loses the section boundaries, so the review scores only what it can
              recognise.
            </span>
          </div>
        }
        ...
        @if (linkedin.review(); as review) {
          ...
          @if (linkedin.wouldBenefitFromExport()) {
            <section class="card advice">
              ...
              <p class="body">
                A profile PDF carries only your <strong>top three skills</strong>, no
                About section and no role descriptions — so parts of this review are
                scoring the export's limits rather than your profile. The official data
                export has all of it.
              </p>
              <p class="body muted">
                <strong>Me → Settings &amp; Privacy → Data Privacy → Get a copy of your
                data.</strong> Pick Profile, Positions, Education and Skills. LinkedIn
                takes up to 72 hours, then upload the ZIP here — it fills in what is
          ...
          @if (review.coverage; as coverage) {
            <section class="card">
              ...
              @if (coverage.missing.length > 0) {
                <p class="body muted">
                  Not evidenced anywhere a recruiter searches — your headline, About or
                  skills:
                </p>
                <ul class="terms">
            ...
            <section class="card">
              <p class="body muted">
                Set your <a routerLink="/goals">target roles</a> and this screen will also
                score how findable the profile is in recruiter search.
              </p>
            </section>
          }
          ...
          <section class="card">
            <div class="card-header">
              ...
              <span class="card-note">
                {{ review.consistency.length }} difference(s)
              </span>
            </div>
            @if (review.consistency.length === 0) {
              <p class="body muted">
                The two records agree on every employer, title and date.
              </p>
            } @else {
              <p class="body muted">
                A recruiter can see both. CV Pal does not change either — which one is
                right is your call.
              </p>
              <ul class="issues">
          ...
          <section class="card">
            ...
            <dl class="facts">
              ...
              <dt>Roles</dt><dd>{{ profile.positions.length }}</dd>
              <dt>Education</dt><dd>{{ profile.educations.length }}</dd>
              <dt>Skills</dt><dd>{{ profile.skills.length }}</dd>
            </dl>
  ...
  styles: [
    `
      ...
      .intro a, .link, .body a { color: var(--accent); font-weight: 600; }
      ...
      .body { font-size: 13px; margin: 0 0 10px; max-width: 72ch; line-height: 1.5; }
      ...
      button.link { background: none; border: 0; padding: 0; font-size: 13px; cursor: pointer; }

      .paste { display: flex; flex-direction: column; gap: 8px; margin-top: 12px; }
      .paste textarea { width: 100%; font: inherit; font-size: 13px; padding: 8px; border-radius: 6px; border: 1px solid var(--border, #ddd); background: transparent; color: inherit; }

      .score-head { display: flex; justify-content: space-between; align-items: baseline; }
      .score-label { font-size: 13px; font-weight: 600; margin-right: 10px; }
      .score-value { font-size: 20px; font-weight: 700; }
      .score-value small { font-size: 12px; font-weight: 500; color: var(--text-tertiary); }
      .badge { font-size: 11px; text-transform: uppercase; letter-spacing: .04em; color: var(--text-tertiary); }
      .score-bar { height: 6px; border-radius: 3px; background: var(--border, #eee); overflow: hidden; margin: 8px 0; }
      .score-fill { height: 100%; background: var(--accent); }
      .note { margin: 0 0 12px; font-size: 12px; color: var(--text-tertiary); max-width: 72ch; }

      .sections { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 8px; }
      .section { display: flex; gap: 12px; font-size: 13px; padding-left: 10px; border-left: 3px solid var(--border, #ddd); }
      .section.missing { border-left-color: var(--danger, #c0392b); }
      .section.thin { border-left-color: var(--warning, #d68910); }
      .section.ok { border-left-color: var(--success, #27ae60); }
      .section-name { font-weight: 600; min-width: 90px; }
      .section-detail { color: var(--text-secondary); }

      .advice h3 { font-size: 15px; margin: 0 0 8px; }

      .terms { list-style: none; margin: 0; padding: 0; display: flex; flex-wrap: wrap; gap: 6px; }
      .term { font-size: 12px; padding: 3px 8px; border-radius: 999px; background: var(--border, #eee); }
      .term.required { font-weight: 700; }

      .issues { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 6px; }
      .issue { font-size: 13px; color: var(--text-secondary); padding-left: 10px; border-left: 3px solid var(--warning, #d68910); }

      .facts { display: grid; grid-template-columns: auto 1fr; gap: 4px 16px; margin: 0 0 12px; font-size: 13px; }
      .facts dt { color: var(--text-tertiary); }
      .facts dd { margin: 0; }
      .remove { background: none; border: 0; padding: 0; font-size: 12px; color: var(--danger, #c0392b); cursor: pointer; }
    `,
  ],
})
export class LinkedInComponent {
  ...
  submitPaste(): void {
    ...
    this.run(() => this.linkedin.paste(text), 'Could not read that text.', () => {
