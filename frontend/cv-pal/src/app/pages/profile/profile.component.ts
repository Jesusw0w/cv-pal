...
@Component({
      ...
      <header class="card identity">
        @if (editing()) {
          <form class="identity-main edit-form" (ngSubmit)="saveProfile()">
            <label>Headline
              <input name="headline" [(ngModel)]="draft.headline" placeholder="Senior Backend Engineer" />
            </label>
            <label>Location
              <input name="location" [(ngModel)]="draft.location" placeholder="Porto, Portugal" />
            </label>
            <label>Phone
              <input name="phone" type="tel" [(ngModel)]="draft.phone" placeholder="+351 912 345 678" />
            </label>
            <label>Summary
              <textarea name="summary" rows="3" [(ngModel)]="draft.summary"></textarea>
            </label>
            <label>LinkedIn
              <input name="linkedin" [(ngModel)]="draft.linkedin_url" placeholder="linkedin.com/in/you" />
            </label>
            <label>Website
        ...
        <div class="identity-main">
          ...
          <p class="identity-meta">
            ...
            &middot; {{ career.experiences().length }} roles
            &middot; {{ career.profile().skills.length }} skills
          </p>
          @if (career.profile().summary; as summary) {
            <p class="identity-summary">{{ summary }}</p>
          }
          <button type="button" class="link edit-link" (click)="startEditing()">Edit details</button>
        </div>
        }
        ...
        <div class="readiness">
          ...
          <div class="readiness-bar" role="progressbar"
      ...
      <section class="card">
        ...
        @if (addingRole()) {
          <form class="add-form" (ngSubmit)="addRole()">
            <div class="add-row">
              <label>Employer<input name="org" [(ngModel)]="roleDraft.organisation" required /></label>
              <label>Title<input name="title" [(ngModel)]="roleDraft.title" required /></label>
            </div>
            <div class="add-row">
              <label>Started<input name="start" type="date" [(ngModel)]="roleDraft.start_date" required /></label>
            ...
            <label>What you did<textarea name="desc" rows="2" [(ngModel)]="roleDraft.description"></textarea></label>
            ...
            <li class="entry">
              @if (editingRole() === role.id) {
                <form class="add-form inline-edit" (ngSubmit)="saveRole(role.id)">
                  <div class="add-row">
                    <label>Employer<input name="org" [(ngModel)]="roleEdit.organisation" required /></label>
                    <label>Title<input name="title" [(ngModel)]="roleEdit.title" required /></label>
                  </div>
                  <div class="add-row">
                    <label>Started<input name="start" type="date" [(ngModel)]="roleEdit.start_date" required /></label>
                    <label>Ended<input name="end" type="date" [(ngModel)]="roleEdit.end_date" /></label>
                    <label>Location<input name="loc" [(ngModel)]="roleEdit.location" /></label>
                  </div>
                  <label>What you did<textarea name="desc" rows="2" [(ngModel)]="roleEdit.description"></textarea></label>
                  <div class="form-actions">
                    <button type="submit" class="primary" [disabled]="busy()">Save</button>
                    <button type="button" class="link" (click)="editingRole.set(null)">Cancel</button>
                  </div>
                </form>
              } @else {
                <div class="entry-head">
                  ...
                  <button type="button" class="edit" [attr.aria-label]="'Edit ' + role.title"
                          (click)="startRoleEdit(role)">Edit</button>
                  <button type="button" class="remove" [attr.aria-label]="'Remove ' + role.title"
                          (click)="removeRole(role.id)">Remove</button>
                </div>
      ...
      <section class="card">
        <div class="card-header">
          ...
          <form class="inline-add" (ngSubmit)="addSkill()">
            <input name="skill" [(ngModel)]="skillDraft" placeholder="Add a skill" aria-label="Skill name" />
            <button type="submit" class="link" [disabled]="busy() || !skillDraft.trim()">Add</button>
          </form>
        ...
        @if (suggestions().length > 0) {
          <div class="suggested">
            <div class="suggested-head">
              <p class="add-note">
                {{ suggestions().length }} skill(s) are named in roles you have already
                described. Applying a suggestion cites those roles as evidence.
              </p>
            ...
            @for (suggestion of suggestions(); track suggestion.skill_id) {
              <div class="suggested-row">
                ...
                <button type="button" class="cite" [disabled]="busy()"
                        (click)="applyEvidence(suggestion)">Apply</button>
              </div>
            }
          </div>
        }
                  ...
                  <li class="skill" [class.unevidenced]="!skill.is_evidenced">
                    ...
                    <button type="button" class="cite"
                      ...
                      @if (skill.is_evidenced) {
                        {{ skill.evidence_experience_ids.length }} role{{ skill.evidence_experience_ids.length > 1 ? 's' : '' }}
                      } @else {
                        <span class="badge badge-amber">cite a role</span>
                      }
                    </button>
                    <button type="button" class="remove" [attr.aria-label]="'Remove ' + skill.name"
                            (click)="removeSkill(skill.id)">&times;</button>
                  </li>

                  @if (evidencing() === skill.id) {
                    <li class="evidence-picker">
                      @if (career.experiences().length === 0) {
                        <p class="add-note">Add a role first — evidence is a role that shows you used this.</p>
                      } @else {
                        <p class="add-note">
                          Which roles did you use {{ skill.name }} in? A generated CV only
                          claims skills with a role behind them.
                        </p>
                        @for (role of career.experiences(); track role.id) {
                          <label class="cite-row">
                            <input type="checkbox" [checked]="skill.evidence_experience_ids.includes(role.id)"
                                   [disabled]="busy()" (change)="toggleEvidence(skill, role.id)" />
      ...
      <section class="card">
        ...
        @if (addingCourse()) {
          <form class="add-form" (ngSubmit)="addCourse()">
            <div class="add-row">
              <label>Institution<input name="inst" [(ngModel)]="courseDraft.institution" required /></label>
              <label>Qualification<input name="qual" [(ngModel)]="courseDraft.qualification" required /></label>
            </div>
            <div class="add-row">
              <label>Field<input name="field" [(ngModel)]="courseDraft.field_of_study" /></label>
              <label>Started<input name="cstart" type="date" [(ngModel)]="courseDraft.start_date" /></label>
              <label>Ended<input name="cend" type="date" [(ngModel)]="courseDraft.end_date" /></label>
            </div>
            ...
            <li class="entry">
              @if (editingCourse() === course.id) {
                <form class="add-form inline-edit" (ngSubmit)="saveCourse(course.id)">
                  <div class="add-row">
                    <label>Institution<input name="inst" [(ngModel)]="courseEdit.institution" required /></label>
                    <label>Qualification<input name="qual" [(ngModel)]="courseEdit.qualification" required /></label>
                  </div>
                  <div class="add-row">
                    <label>Field<input name="field" [(ngModel)]="courseEdit.field_of_study" /></label>
                    <label>Started<input name="cstart" type="date" [(ngModel)]="courseEdit.start_date" /></label>
                    <label>Ended<input name="cend" type="date" [(ngModel)]="courseEdit.end_date" /></label>
                  </div>
                  <div class="form-actions">
                    <button type="submit" class="primary" [disabled]="busy()">Save</button>
                    <button type="button" class="link" (click)="editingCourse.set(null)">Cancel</button>
                  </div>
                </form>
              } @else {
                <div class="entry-head">
                  <span class="entry-title">{{ course.qualification }}
                    @if (course.field_of_study) { &mdash; {{ course.field_of_study }} }
                  </span>
                  <button type="button" class="edit" [attr.aria-label]="'Edit ' + course.qualification"
                          (click)="startCourseEdit(course)">Edit</button>
                  <button type="button" class="remove" [attr.aria-label]="'Remove ' + course.qualification"
                          (click)="removeCourse(course.id)">Remove</button>
                </div>
                ...
                <span class="entry-dates">
                  ...
                  @if (course.grade) { &middot; {{ course.grade }} }
                </span>
              }
            </li>
          } @empty {
  ...
  styles: [
    `
      ...
      .identity { display: flex; gap: 24px; justify-content: space-between; padding: 24px; }
      .identity h1 { font-size: 22px; line-height: 1.25; margin: 0 0 4px; }
      .identity-meta { margin: 0; font-size: 13px; color: var(--text-tertiary); }
      .identity-summary { margin: 12px 0 0; font-size: 14px; color: var(--text-secondary); max-width: 70ch; }

      .readiness { display: flex; flex-direction: column; align-items: flex-end; min-width: 140px; }
      .readiness-value { font-size: 28px; font-weight: 700; line-height: 1; }
      .readiness-value small { font-size: 14px; font-weight: 500; color: var(--text-tertiary); }
      .readiness-label { font-size: 12px; color: var(--text-tertiary); }
      .readiness-bar {
        width: 100%; height: 6px; margin-top: 10px; border-radius: 999px;
        background: var(--border-light); overflow: hidden;
      }
      .readiness-fill { height: 100%; background: var(--accent); }

      .card-note { font-size: 12px; color: var(--text-tertiary); }

      .edit-form, .add-form { display: flex; flex-direction: column; gap: 10px; flex: 1; }
      .add-form { padding: 12px 18px; border-bottom: 1px solid var(--border-light); }
      .add-row { display: flex; flex-wrap: wrap; gap: 10px; }
      .add-row label { flex: 1 1 160px; }
      .edit-form label, .add-form label {
        display: flex; flex-direction: column; gap: 4px;
        font-size: 12px; color: var(--text-tertiary);
      }
      .edit-form input, .edit-form textarea,
      .add-form input, .add-form textarea, .inline-add input {
        padding: 8px 10px; border: 1px solid var(--border-light); border-radius: var(--radius);
        background: var(--bg-primary); color: var(--text-primary);
        font-size: 13px; font-family: inherit;
      }
      .add-note { margin: 0; font-size: 12px; color: var(--text-tertiary); }
      .form-actions { display: flex; align-items: center; gap: 12px; }
      .primary {
        align-self: flex-start; padding: 8px 16px; border-radius: var(--radius);
        background: var(--accent); color: #fff; font-size: 13px; font-weight: 600;
      }
      .primary:disabled { opacity: .5; }
      .link { font-size: 13px; font-weight: 600; color: var(--accent); }
      .edit-link { align-self: flex-start; margin-top: 10px; }
      .error { margin: 0; font-size: 13px; color: var(--danger, #dc2626); }

      .inline-add { display: flex; gap: 6px; align-items: center; }
      .inline-add input { font-size: 12px; padding: 4px 8px; width: 130px; }

      .remove {
        margin-left: auto; font-size: 12px; color: var(--text-tertiary); padding: 2px 6px;
      }
      .remove:hover { color: var(--danger, #dc2626); }

      .notice { margin: 0; padding: 14px 18px; font-size: 13px; color: var(--text-secondary); }
      .retry { margin-left: 8px; color: var(--accent); font-weight: 600; }

      .gap-list { list-style: none; margin: 0; padding: 8px; }
      .gap { display: flex; flex-direction: column; gap: 2px; padding: 10px 12px; border-radius: var(--radius); }
      .gap:hover { background: var(--bg-hover); }
      .gap-label { font-size: 14px; font-weight: 600; }
      .gap-detail { font-size: 12px; color: var(--text-tertiary); }
      ...
      .entry-org { font-size: 13px; color: var(--accent); }
      .entry-dates { font-size: 12px; color: var(--text-tertiary); }
      .entry-body { margin: 6px 0 0; font-size: 13px; color: var(--text-secondary); max-width: 80ch; }
      .empty { color: var(--text-tertiary); font-size: 13px; }

      .suggested {
        margin: 0 18px 4px; padding: 10px 12px;
        border: 1px solid var(--border-light); border-left: 2px solid var(--accent);
        border-radius: var(--radius);
      }
      .suggested-head { display: flex; align-items: baseline; justify-content: space-between; gap: 12px; }
      .suggested-head .add-note { margin: 0; }
      .suggested-row {
        display: flex; align-items: center; gap: 12px; padding: 6px 0;
        border-top: 1px solid var(--border-light);
      }
      .suggested-skill { font-size: 13px; font-weight: 600; }
      .suggested-roles { flex: 1; min-width: 0; font-size: 12px; color: var(--text-tertiary); }

      .skill-groups {
        padding: 12px; display: grid; gap: 16px;
      ...
      .skill { display: flex; align-items: center; gap: 8px; font-size: 14px; }
      .skill.unevidenced .skill-name { color: var(--text-secondary); }
      .skill-years { font-size: 12px; color: var(--text-tertiary); }

      .cite { margin-left: auto; font-size: 12px; color: var(--text-tertiary); }
      ...
      .cite-row input { accent-color: var(--accent); }
      .edit { font-size: 12px; color: var(--text-tertiary); }
      .edit:hover { color: var(--accent); }
      .inline-edit { padding: 4px 0 12px; border: 0; }

      @media (max-width: 768px) {
        .page { padding: 16px; }
        .identity { flex-direction: column; gap: 16px; }
        .readiness { align-items: flex-start; }
      }
    `,
  ],
})
export class ProfileComponent {
  ...
  toggleEvidence(skill: SkillResponse, experienceId: number): void {
    ...
    const next = cited.includes(experienceId)
      ...
      : [...cited, experienceId];
    this.run(
      this.career.updateSkill(skill.id, { evidence_experience_ids: next }),
      () => undefined,
    );
  }
  ...
  applyEvidence(suggestion: EvidenceSuggestionResponse): void {
    ...
    const cited = [
      ...new Set([...skill.evidence_experience_ids, ...suggestion.experience_ids]),
    ];
