...
| Analysis returns "language model is unavailable" | Most often the model named in `CV_PAL_LLM_MODEL` is not installed — `ollama list` shows what is. Otherwise check Ollama is running (`docker compose ps`), or that the hosted API key is valid. The AI Tools screen says which, and `docker compose logs backend` shows the start-up check. The stack reports healthy either way, since everything else works without a model. |
